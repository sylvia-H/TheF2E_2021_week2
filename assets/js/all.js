...
function getCurrentPos_route() {
  ...
  function success(pos) {
    ...
    routeMap.setView([pos_lat, pos_lng], 16); // let routeMap = L.map('map_show', {
    //     center: [pos_lat,pos_lng],
    //     zoom: 16
    // })
    // 載入圖磚
    ...
    L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}', {
      ...
      zoomOffset: -1,
      accessToken: 'pk.eyJ1Ijoic3lsdmlhaC10dyIsImEiOiJja3c2Nzl2eHUweXlmMnFtZTRjb2VjcDQxIn0.Gx5M_zr8rDs4T-jawF2UDQ'
    }).addTo(routeMap); // 目前所在位置 GPS 圖示內容
    ...
    L.marker([pos_lat, pos_lng], {
      icon: posGPSIcon
    }).addTo(routeMap); // 顯示所在位置 GPS 圖示 lottie 動畫
...
function getCurrentPos_spot() {
  ...
  function success(pos) {
    ...
    spotMap.setView([pos_lat, pos_lng], 16); // let spotMap = L.map('map_show', {
    //     center: [pos_lat,pos_lng],
    //     zoom: 16
    // })
    // 載入圖磚
    ...
    L.marker([pos_lat, pos_lng], {
      icon: posGPSIcon
    }).addTo(spotMap); // 顯示所在位置 GPS 圖示 lottie 動畫
